...
- 以**强制模式**运行 **WAF**(`deny` → 403,`challenge` → 429),引擎侧计数命中并
  记录安全事件;
...
固定窗口)。`geo` 需要 GeoIP 库,永不匹配。规则按 priority 升序求值、首个命中生效,
否则回落到默认动作。**数据面强制执行**;管理台永不被求值(所以不会把自己锁在外面)。
...
| 场景 | 单请求开销 | 单核吞吐 |
| ---- | ---------- | -------- |
| 良性(全部规则求值) | ~440 ns/req | ~230 万 req/s |
| 攻击(命中即返回) | ~210 ns/req | ~470 万 req/s |
